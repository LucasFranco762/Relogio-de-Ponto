...
class SettingsService:
    ...
    def save(self, empresa: str, inicio: time, fim: time, modo: str, limite: int, horario_corte: time | None = None) -> None:
        with self.database.session() as session:
            ...
            if horario_corte is not None:
                item.horario_corte = horario_corte
            session.add(item)
